...
RecordBlockingKeyGenerator<VideoGames, Attribute>{
	...
	@Override
	public void generateBlockingKeys(VideoGames record, Processable<Correspondence<Attribute, Matchable>> correspondences,
			DataIterator<Pair<String, VideoGames>> resultCollector) {
		resultCollector.next(new Pair<>(Integer.toString(record.getDate()), record));
	}
